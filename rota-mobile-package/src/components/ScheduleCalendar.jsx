...
function GridCard({ data, isToday, scheduleDate, onClick, hasSelectedStaff, dimmed, overrideActive }) {
  ...
  return (
    <div
      id={`day-card-${dayStr}`}
      onClick={onClick}
      className={`bg-[#131520] border border-white/[0.07] rounded-2xl flex flex-col h-full overflow-hidden cursor-pointer transition-all ${dimmed ? 'opacity-30' : 'opacity-100'} hover:border-[#00D66B]/30 hover:shadow-lg`}
      style={{
        border: isToday ? "2px solid #00D66B" : (hasSelectedStaff ? "1px solid rgba(255,255,255,0.25)" : undefined),
      }}
    >
      <div className="flex items-center justify-between px-3 pt-2 pb-1 shrink-0">
        <div className="flex items-baseline gap-1">
          <span style={{ fontSize: 13, color: isToday ? "#00D66B" : "#c8d4e8", fontWeight: isToday ? 700 : 600 }}>
            {dayStr}
          </span>
          {isToday && (
            <span style={{ fontSize: 8, color: "#00D66B", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>
              Today
            </span>
          )}
          {overrideActive && (
      ...
      <div className="flex flex-1 gap-px px-2 pb-2 min-h-0">
        ...
        <div style={{ width: 1.5, backgroundColor: "rgba(255,255,255,0.1)", alignSelf: "stretch", margin: "0 4px" }} />
        <div className="flex-1 flex flex-col gap-1 min-w-0 overflow-hidden">
...
function MobileCard({ data, isToday, scheduleDate, onClick, hasSelectedStaff, dimmed, overrideActive }) {
  ...
  return (
    <div
      id={`day-card-${dayStr}`}
      onClick={onClick}
      className={`bg-[#131520] border border-white/[0.07] rounded-2xl overflow-hidden cursor-pointer transition-all ${dimmed ? 'opacity-30' : 'opacity-100'}`}
      style={{
        border: isToday ? "2px solid #00D66B" : (hasSelectedStaff ? "1px solid rgba(255,255,255,0.25)" : undefined),
      }}
    >
      <div className="flex">
        <div
          ...
          style={{
            width: 60,
            padding: "12px 0",
            backgroundColor: isToday ? "rgba(0, 214, 107, 0.15)" : "rgba(255,255,255,0.025)",
        ...
        >
          ...
          <span style={{ fontSize: 26, lineHeight: 1, color: isToday ? "#00D66B" : "#c8d4e8", fontWeight: 800, fontVariantNumeric: "tabular-nums" }}>
            {dayStr}
          </span>
          <span style={{ fontSize: 10, color: isToday ? "#00D66B" : "#8B8E97", fontWeight: 600, marginTop: 2, textTransform: "uppercase", letterSpacing: "0.06em" }}>
      ...
      {data.NT.length > 0 ? (
        <div className="flex items-center gap-3 px-4 py-2.5" style={{ backgroundColor: ntColor?.bar || '#232429', borderTop: "1px solid rgba(0,0,0,0.15)" }}>
          ...
          <div>
            <div className="grid grid-cols-7 gap-2 mb-2">
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((d, i) => (
                <div key={d} className="text-center py-1" style={{ fontSize: 10, color: (i === 0 || i === 6) ? "#54565F" : "#8B8E97", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase" }}>
                  {d}
                </div>
              ))}
            </div>
