...
const AppShell = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-background text-foreground overflow-hidden font-sans">
      {/* Desktop Sidebar (hidden in tab mode — BottomNav handles navigation) */}
      {!isTemplateRoute && (
      ...
      {/* Mobile Top Bar (hidden in tab mode — BottomNav handles navigation) */}
      {!isTemplateRoute && (
      ...
      <main className="flex-1 flex flex-col min-w-0 relative">
        <div className={`flex-1 overflow-auto flex flex-col transition-all duration-500 ${
          isTemplateRoute ? "pt-0 pb-28" : "pt-16 lg:pt-0"
        }`}>
          <AnimatePresence>
            <motion.div
      ...
      {/* Mobile Sidebar Overlay (hidden in tab mode) */}
      {!isTemplateRoute && (
        <AnimatePresence>
          {isMobileSidebarOpen && (
            <>
