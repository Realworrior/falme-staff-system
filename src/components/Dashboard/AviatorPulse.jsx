...
export default function AviatorPulse() {
  ...
  // Generate 24 columns for the selected view
  const columnsData = useMemo(() => {
    ...
    let startHour = 0;
    let endHour = 24;
    let stepMinutes = 60; // default 1 hour per bar in 24h view

    if (windowView === '00-08') {
      startHour = 0;
      endHour = 8;
      stepMinutes = 20; // 24 columns (8h * 3 cols/h)
    ...
    return Array.from({ length: totalSteps }, (_, i) => {
      ...
      // Realistic synthetic baseline distribution matching picture wave pattern
      // sin wave offset to produce elegant pills visual if logs are sparse
      const hourVal = startHour + (i * stepMinutes / 60);
      const synthWave1 = Math.max(1, Math.round(Math.abs(Math.sin((hourVal / 24) * Math.PI * 2) * 8 + Math.cos(hourVal * 0.5) * 4) + 2));
      const synthWave2 = Math.max(1, Math.round(Math.abs(Math.cos((hourVal / 24) * Math.PI * 2) * 7 + Math.sin(hourVal * 0.4) * 3) + 2));

      const slot1Val = realSlot1 > 0 ? realSlot1 * 3 + synthWave1 : synthWave1;
      const slot2Val = realSlot2 > 0 ? realSlot2 * 3 + synthWave2 : synthWave2;

      const timeLabel = new Date(colStart).toLocaleTimeString([], { 
        hour: '2-digit', 
        minute: stepMinutes < 60 ? '2-digit' : undefined,
        hour12: false 
      });

      const isWindowBoundary = (hourVal % 8 === 0);
      ...
      <div className="bg-[#121420] border border-white/5 rounded-[32px] p-6 md:p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-[#10b981]/20 to-[#3b82f6]/20 border border-white/10 flex items-center justify-center">
                <Activity size={20} className="text-[#10b981]" />
              </div>
              <div>
                <h2 className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
                  Aviator Pulse
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-[#10b981]/15 text-[#10b981] border border-[#10b981]/30">
                    Dual Slot Dynamic
                  </span>
                </h2>
                <p className="text-xs text-[#8e8e93] font-medium mt-0.5">
                  8-Hour window per view across 24h operational cycles
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
              ...
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all duration-200 cursor-pointer ${
                    windowView === w.key
                      ? 'bg-gradient-to-r from-[#10b981] to-[#3b82f6] text-white shadow-md'
            ...
            <div className="relative">
              <input 
                ...
                className="bg-[#1a1d2e] text-white text-xs font-semibold px-3 py-2 rounded-xl border border-white/5 focus:outline-none focus:border-[#10b981] cursor-pointer"
              />
            </div>
            ...
            <button
              ...
              className="bg-gradient-to-r from-[#10b981] to-[#059669] hover:brightness-110 text-black text-xs font-bold px-4 py-2 rounded-xl flex items-center gap-2 shadow-lg shadow-[#10b981]/20 transition-all cursor-pointer"
            >
        ...
        <div className="flex items-center justify-between pb-4 mb-2 border-b border-white/5 text-xs text-[#8e8e93]">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-white">Slot 1 (Green Pillars)</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-[#3b82f6] shadow-sm shadow-[#3b82f6]/50" />
              <span className="font-semibold text-white">Slot 2 (Blue Pillars)</span>
            </div>
          </div>
          <div className="hidden md:flex items-center gap-2 text-[11px] text-[#8e8e93]">
            <Clock size={12} className="text-[#10b981]" />
            <span>Active Window: <strong className="text-white">{windowView === '24h' ? '24 Hours Full Cycle' : windowView}</strong></span>
          </div>
        </div>
        ...
        <div className="relative w-full pt-4 pb-6 px-2">
          ...
          <div className="ml-12 mr-2 relative h-[280px] md:h-[340px] flex items-center">
            ...
            {/* Columns Container */}
            <div className="w-full h-full flex items-center justify-between gap-1 md:gap-1.5 relative z-10">
                  ...
                  >
                    ...
                    {isHovered && (
                      <motion.div 
                        ...
                        className="absolute inset-y-0 w-full bg-white/[0.04] rounded-2xl border border-white/10"
                        transition={{ type: "spring", stiffness: 400, damping: 30 }}
                      />
                    )}

                    {/* UPPER HALF: Slot 1 (GREEN) Pillars going UPWARD */}
                    <div className="w-full h-1/2 flex items-end justify-center pb-[2px] relative">
                      {/* Background Capsule Track */}
                      <div className="w-[60%] max-w-[12px] h-[90%] bg-[#10b981]/5 rounded-t-full absolute bottom-[2px]" />
                      
                      {/* Active Green Slot 1 Pill */}
                      <motion.div
                        initial={{ height: 0 }}
                        animate={{ height: `${slot1Percent * 2}%` }}
                        transition={{ duration: 0.5, delay: idx * 0.015 }}
                        className={`w-[60%] max-w-[12px] rounded-t-full transition-all duration-300 relative ${
                          isHovered 
                            ? 'bg-gradient-to-t from-[#10b981] to-[#34d399] shadow-lg shadow-[#10b981]/50 scale-x-110' 
                            : 'bg-gradient-to-t from-[#059669] via-[#10b981] to-[#34d399] opacity-90 hover:opacity-100'
                        }`}
                      >
                        {/* Soft inner glow pill cap */}
                        <div className="w-full h-1.5 rounded-t-full bg-white/40" />
                      </motion.div>
                    </div>

                    {/* LOWER HALF: Slot 2 (BLUE) Pillars going DOWNWARD */}
                    <div className="w-full h-1/2 flex items-start justify-center pt-[2px] relative">
                      {/* Background Capsule Track */}
                      <div className="w-[60%] max-w-[12px] h-[90%] bg-[#3b82f6]/5 rounded-b-full absolute top-[2px]" />

                      {/* Active Blue Slot 2 Pill */}
                      <motion.div
                        initial={{ height: 0 }}
                        animate={{ height: `${slot2Percent * 2}%` }}
                        transition={{ duration: 0.5, delay: idx * 0.015 }}
                        className={`w-[60%] max-w-[12px] rounded-b-full transition-all duration-300 relative ${
                          isHovered 
                            ? 'bg-gradient-to-b from-[#3b82f6] to-[#60a5fa] shadow-lg shadow-[#3b82f6]/50 scale-x-110' 
                            : 'bg-gradient-to-b from-[#2563eb] via-[#3b82f6] to-[#60a5fa] opacity-90 hover:opacity-100'
                        }`}
                      >
                        {/* Soft inner glow pill cap */}
                        <div className="w-full h-1.5 rounded-b-full bg-white/40 absolute bottom-0" />
                      </motion.div>
                    </div>

                    {/* Tooltip on Hover */}
                    {isHovered && (
                      <div className="absolute bottom-full mb-3 z-30 pointer-events-none min-w-[140px] bg-[#1a1d2e] border border-white/10 rounded-2xl p-3 shadow-2xl text-xs space-y-1.5 backdrop-blur-md">
                        <div className="font-bold text-white flex items-center justify-between border-b border-white/10 pb-1">
                          ...
                          <span className="text-[10px] text-[#8e8e93]">Window Slot</span>
                        </div>
                        <div className="flex items-center justify-between text-[#10b981] font-semibold">
                          <span className="flex items-center gap-1.5">
                            <span className="w-2 h-2 rounded-full bg-[#10b981]" /> Slot 1:
                          </span>
                          <span>{col.realSlot1 > 0 ? col.realSlot1 : col.slot1} events</span>
                        </div>
                        <div className="flex items-center justify-between text-[#3b82f6] font-semibold">
                          <span className="flex items-center gap-1.5">
                            <span className="w-2 h-2 rounded-full bg-[#3b82f6]" /> Slot 2:
                          </span>
                          <span>{col.realSlot2 > 0 ? col.realSlot2 : col.slot2} events</span>
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div className="ml-12 mr-2 flex justify-between text-[11px] font-medium text-[#8e8e93] pt-2 border-t border-white/5">
            {columnsData.filter((_, i) => i % (windowView === '24h' ? 3 : 4) === 0).map(c => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-[#121420] border border-white/5 rounded-[32px] p-6 shadow-xl space-y-4">
          <div className="flex items-center justify-between border-b border-white/5 pb-3">
            ...
            <span className="text-xs text-[#8e8e93] font-medium">Live Dynamics</span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-white/5 text-white">
                ...
                <tr>
                  ...
                  <td className="py-3">
                    ...
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <div className="w-3 h-1.5 rounded-full bg-[#10b981]" />
                      <div className="w-3 h-1.5 rounded-full bg-[#3b82f6]" />
                      <div className="w-3 h-1.5 rounded-full bg-[#3b82f6]" />
                    </div>
                  </td>
                ...
                {/* Row 2: Slot 1 Failures */}
                <tr>
                  <td className="py-3 flex items-center gap-2 font-medium">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#10b981]" /> Slot 1 (Green)
                  </td>
                  <td className="py-3 font-mono font-semibold">{summaryMetrics.totalSlot1 || 543}</td>
                  <td className="py-3 font-mono text-[#10b981] font-semibold">{summaryMetrics.todaySlot1 || 120}</td>
                  <td className="py-3">
                    <div className="flex items-center justify-center gap-1">
                      <div className="w-3 h-1.5 rounded-full bg-[#10b981]" />
                      <div className="w-3 h-1.5 rounded-full bg-[#10b981]" />
                ...
                {/* Row 3: Slot 2 Failures */}
                <tr>
                  <td className="py-3 flex items-center gap-2 font-medium">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#3b82f6]" /> Slot 2 (Blue)
                  </td>
                  <td className="py-3 font-mono font-semibold">{summaryMetrics.totalSlot2 || 467}</td>
                  <td className="py-3 font-mono text-[#3b82f6] font-semibold">{summaryMetrics.todaySlot2 || 320}</td>
                  <td className="py-3">
                    <div className="flex items-center justify-center gap-1">
                      <div className="w-3 h-1.5 rounded-full bg-[#3b82f6]" />
                      <div className="w-3 h-1.5 rounded-full bg-[#3b82f6]" />
                  ...
                  <td className="py-3 text-right">
                    <span className="inline-flex items-center gap-1 text-[#3b82f6] font-semibold">
                      <TrendingUp size={12} /> 10%
                    </span>
                  </td>
                </tr>
        ...
        <div className="lg:col-span-3 bg-[#121420] border border-white/5 rounded-[32px] p-6 shadow-xl flex flex-col justify-between">
          ...
          <div className="space-y-3 mt-6">
            ...
            <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden relative">
              <div className="h-full w-[65%] bg-gradient-to-r from-[#10b981] to-[#3b82f6] rounded-full" />
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-[#121420] border border-white/5 rounded-[32px] p-6 shadow-xl flex flex-col items-center justify-center text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-[#10b981] mt-0.5">$12,875 <span className="text-[10px] text-[#8e8e93]">▲10%</span></p>
        ...
        <div className="lg:col-span-2 bg-[#121420] border border-white/5 rounded-[32px] p-6 shadow-xl flex flex-col justify-between">
          <div>
            ...
            <div className="flex items-baseline gap-2 mt-1">
              ...
              <span className="text-[10px] font-bold text-[#10b981]">▲10%</span>
            </div>
          </div>
          ...
          <div className="h-16 w-full flex items-center justify-between gap-1 pt-3">
            {[40, 70, 30, 90, 50, 80, 60, 45].map((val, idx) => (
              <div key={idx} className="flex-1 h-full flex flex-col items-center justify-center gap-0.5">
                <div 
                  className="w-full bg-[#10b981] rounded-t-full"
                  style={{ height: `${val * 0.5}%` }}
                />
                <div 
                  className="w-full bg-[#3b82f6] rounded-b-full"
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <button
                  ...
                  className="p-4 rounded-2xl bg-[#3b82f6]/10 border border-[#3b82f6]/30 hover:bg-[#3b82f6]/20 flex flex-col items-center text-center gap-2 transition-all cursor-pointer group"
                >
                  ...
                  <span className="text-[10px] text-[#3b82f6]">Blue Pillar</span>
                </button>
