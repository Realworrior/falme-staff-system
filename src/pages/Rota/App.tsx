...
const generateICSContent = (selectedStaff: string, currentDate: Date, schedule: any[]) => {
  ...
  staffSchedule.forEach((day, index) => {
    ...
    const shiftColor = shiftType === 'AM' ? '#2DD4BF' : shiftType === 'PM' ? '#60A5FA' : '#FBBF24';
    ...
    // Structured Description for iPhone
    const description = [
      `SHIFT TYPE: ${shiftType} ${icon}`,
      `DATE: ${format(day.date, 'PPPP')}`,
      `--------------------------`,
      `TEAM MATES ON THIS SHIFT:`,
      mates.length ? mates.map(m => `• ${m}`).join('\\n') : 'Solo Shift',
      `--------------------------`,
      `Generated by Falme AI Staff Matrix`
    ].join('\\n');

    ics.push(`DESCRIPTION:${description}`);
    ics.push(`LOCATION:Falme Operations Center`);
    ics.push(`STATUS:CONFIRMED`);
    ics.push(`TRANSP:OPAQUE`);
    ics.push(`CATEGORIES:${shiftType},FalmeRota`);
    
    // Apple-specific coloring
    ics.push(`X-APPLE-CALENDAR-COLOR:${shiftColor}`);
    
    // Standard and Apple fallback colors
